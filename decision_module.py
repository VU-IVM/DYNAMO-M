...
class DecisionModule:
    ...
    def sample_error_terms(self, n_agents, regions_select):
        ...
        self.error_terms_stay = self.model.random_module.random_state.uniform((1-self.error_interval), (1+self.error_interval), size=n_agents)
        self.error_terms_migrate = self.model.random_module.random_state.uniform((1-self.error_interval), (1+self.error_interval), size=(regions_select.size, n_agents))

    def load_gravity_models(self):
        '''This function loads the gravity models for each admin unit in the model run.'''
        # get coutry id
        world_regions_in_model = np.unique([household.UN_region_id for household in self.agents.regions.all_households])
        self.gravity_models = {region: read_gravity_model(region=region) for region in world_regions_in_model}
    ...
    def IterateThroughFlood(
        ...
        NPV_t0 = (
            wealth +
            income +
            amenity_value).astype(
            np.float32)  # no flooding in t=0
    ...
    def calcEU_adapt(
        ...
        **kwargs
        ...
        if (NPV_summed == 1).any():
            ...
            print(f'[calcEU_adapt] Warning, {n_negative} negative NPVs encountered in {geom_id} ({np.round(n_negative/NPV_summed.size*100, 2)}%)')
    ...
    def calcEU_insure(
        ...
        **kwargs
        ...
        if (NPV_summed == 1).any():
            n_negative = np.sum(NPV_summed == 1)
            print(f'[calcEU_insure] Warning, {n_negative} negative NPVs encountered in {geom_id} ({np.round(n_negative/NPV_summed.size*100, 2)}%)')
    ...
    def calcEU_do_nothing(
        ...
        **kwargs
        ...
        if (NPV_summed == 1).any():
            n_negative = np.sum(NPV_summed == 1)
            print(f'[calcEU_do_nothing] Warning, {n_negative} negative NPVs encountered in {geom_id} ({np.round(n_negative/NPV_summed.size*100, 2)}%)')
    ...
    def match_amenity_premium(
        ...
        for premium, count in zip(unique_current_premiums, counts):
            ...
            if cells_with_similar_premium.size > 0:
                ...
                cells_within_mask = fast_intersect(allocation_mask, cells_with_similar_premium)

                if self.model.args.run_with_checks:
                    # check if fast intersect works as it should
                    cells_within_mask_1 = np.intersect1d(allocation_mask, cells_with_similar_premium)
                    assert all(cells_within_mask == cells_within_mask_1)

                if cells_within_mask.size > 0:
                    cells_with_similar_premium = cells_within_mask
                    agent_idx = np.where(current_amenity_premium==premium) # indices of the agents with the premium we are iterating over
                    sample = self.model.random_module.random_state.choice(cells_with_similar_premium, size=(5, count)) # sample 5 random cells
                    premium_sample = amenity_premium_cells_region[sample]  # get the amenity premium of these cells
                    risk_sample = damage_factor_region[sample] # get the risk in these cells
                    risk_sample *= risk_perception[agent_idx] # adjust for risk perceptions
                    # take best cells
                    best_cells = (premium_sample - risk_sample).argmax(axis=0) # subtract from amenity premium to determine best locations
                    cells_to_assess[agent_idx] = sample[best_cells, np.arange(sample.shape[1])]
            else:
                # if no matching premiums are found return -1. Will be used as 'flag'
                cells_to_assess = np.full(cells_to_assess.size, -1, cells_to_assess.dtype)
    ...
    def fill_regions(
        ...
        for i, region in enumerate(regions_select):
            # Sort income
            sorted_income = income_distribution_regions[region] # should be sorted after creation
            # Derive indices from percentiless
            income_indices = np.floor(
                income_percentile *
                0.01 *
                len(sorted_income)).astype(
                np.int32)
            ...
            expected_wealth_agents[i] = np.take(self.income_wealth_ratio, income_percentile) * expected_income_agents[i]

            # sample amenity premiums
            assert amenity_premium_regions[region].shape == snapshot_damages_cells[region].shape
            # iterate to find matching amenity premiums in destination
            cells_to_assess = np.full(n_agents, -1, np.int32)                   
            if region in self.agents.regions.amenity_premium_indices.keys() and amenity_premium_regions[region].size > 1:

                # get allocation mask to filter out full cells
                allocation_mask = self.agents.regions.all_households[region].mask_household_allocation

                # get indices of amenity values destination region
                amenity_indices_destination = self.agents.regions.amenity_premium_indices[region]
                               
                # get unique bins
                unique_current_amenity_bins, counts = np.unique(current_amenity_premium, return_counts=True)
                ...
                # get amenity premium
                amenity_premium_sampled_cells = amenity_premium_regions[region][cells_to_assess]               
                ...
                expected_amenity_value = amenity_premium_sampled_cells * amenity_weight * expected_wealth_agents[i]
    ...
    def fill_NPVs(
        ...
        if sigma == 1:
            ...
            EU_regions[NPV_summed < 0] = -np.inf # just to make sure nobody moves to negative NPVs
    ...
    def EU_migrate(
        ...
        cells_assessed =  np.full(
            (self.model.agents.regions.n, income_percentile.size), -1, dtype=np.int32) # now this array is a bit large, could also work with relative indices
    ...
    def gravity_model(
        ...
        if self.model.settings['gravity_model']['run_with_gravity_model'] and not self.model.args.run_without_gravity_model:
            gravity_settings = self.gravity_models[region_id] # read gravity model settings
            ...
            if flow > pop_i:
                flow = pop_i
                self.model.logger.info(f'Number of movers exceeded population size in gravity model {geom_id}')
            # account for max pop growth in settings
            if self.model.settings['decisions']['migration']['limit_admin_growth']:
                flow = np.min([flow, self.model.settings['decisions']['migration']['max_admin_growth'] * pop_j]) 
        else: flow = 0
        return int(flow)
